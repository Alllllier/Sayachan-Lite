...
| Backend Notes route/service/model | `backend/src/routes/notesRoutes.js`、`backend/src/services/notesService.js`、`backend/src/models/Note.js` | `keep` | scoped note CRUD、pinning、archive/restore、note-origin task cascades 的持久 route/service/model 分离。 | service behavior 拥有 archive cascade semantics；route validation 未来可被替换。 |
| Backend Projects route/service/model | `backend/src/routes/projectsRoutes.js`、`backend/src/services/projectsService.js`、`backend/src/models/Project.js` | `keep` | scoped project CRUD、status、focus task id、pinning、archive/restore、project-task cascades 的持久分离。 | focus/archive behavior 是 architecture-sensitive。 |
