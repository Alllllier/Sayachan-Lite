...
| `discussion_batch_008` | `follow-up` | `Post-simplification residue cleanup and testing-baseline follow-up for task/project/note` | `Keep the completed runtime residue cleanup and validation split-out as historical context; backend testing, frontend behavior coverage, browser/UI review, and fallback-command policy now live in narrower follow-ups.` | `stable` | `2026-05-03` | `runtime cleanup completed; backend testing split to 009; frontend display/coverage split through 010/013; browser/UI review split to 015` |
...
| `discussion_batch_010` | `follow-up` | `Frontend display semantics and test coverage buildout` | `Keep the completed display-semantics line as historical context; panel behavior coverage moved to discussion_batch_013 and repo-native UI review moved to discussion_batch_015.` | `stable` | `2026-05-03` | `slice-001 completed; slice-002 split to 013 and completed; slice-003 split to 015` |
...
| `discussion_batch_015` | `follow-up` | `Repo-native browser and UI review baseline` | `Keep the completed Notes, Projects, Dashboard, and Chat UI review baselines as stabilized validation/tooling context; remaining harness and runtime-baseline follow-ups live in idea_backlog.` | `stable` | `2026-05-04` | `all five slices completed; no active execution-ready slice remains in this batch` |
| `discussion_batch_016` | `follow-up` | `Backend minimal layering and validation architecture` | `Keep the completed service extraction, validation/error-boundary, and route module split as landed context; broader app/server split and config ownership are deferred until real pressure appears.` | `stable` | `2026-05-04` | `slice-001 completed; slice-002 completed; slice-003 completed; slice-004 deferred` |
| `discussion_batch_017` | `follow-up` | `Owner-led auth and invite-gated tester accounts` | `Closed as stable source context after the auth/invite decision, owner skeleton, account isolation, deployed-session fixes, and auth/owner UI review coverage all landed.` | `stable` | `2026-05-04` | `completed; future auth/account work should open from remaining backlog follow-ups or a new discussion` |
| `discussion_batch_018` | `follow-up` | `Engineering modernization path after MVP guardrails` | `Keep the broader modernization discussion active after closing the TypeScript target mapping audit, Engineering Quality Gate V1, type-aware JS pilots, Zod runtime-schema pilots, route schema placement, and the Notes parsed-body pilot.` | `active` | `2026-05-06` | `slice-003 completed through Parsed Body Pilot: Notes Boundary; Parsed Body Rollout for Projects/Tasks is current candidate; slice-001, slice-002, and slice-008 completed; slices 004-007 remain discussion-shaped` |
