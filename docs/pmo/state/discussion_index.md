...
| `discussion_batch_018` | `follow-up` | `Engineering modernization path after MVP guardrails` | `Use the backend TS/ESM migration lessons and type-aware lint findings to shape frontend TS next steps plus backend hardening slices without bundling unrelated architecture cleanup into quality-gate work.` | `active` | `2026-05-07` | `backend TS/ESM migration and Backend TS Quality Gate Cleanup completed; frontend TS remains open; backend hardening follow-ups split into AI bridge contract, public DTO tightening, route state accessor refinement, and type-aware ESLint readiness` |
