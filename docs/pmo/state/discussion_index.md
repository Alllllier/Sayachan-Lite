...
| `discussion_batch_014` | `follow-up` | `Frontend feature-layer split for Projects, Notes, and Dashboard` | `Shape the next architecture migration from large SFCs toward feature API, rules, and composable/controller layers; treat current Dashboard saved-task business as formal, not temporary pending old AI workflow revival.` | `active` | `2026-04-26` | `shaping; likely promote one bounded feature-layer split at a time after code review` |
