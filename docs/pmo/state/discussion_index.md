...
| `discussion_batch_010` | `follow-up` | `Frontend display semantics and test coverage buildout` | `Keep the now-landed project-surface semantics and micro-fix line as context while the next likely move shifts toward frontend panel behavior coverage before repo-native UI review baseline work.` | `active` | `2026-04-21` | `slice-001 landed; slice-002 now most likely next` |
| `discussion_batch_011` | `follow-up` | `Frontend style baseline refactor` | `Keep the landed structure/baseline, controls-core, action-grouping, secondary-controls, and input-state passes as context while legacy cleanup and higher-level panel/shell language remain open.` | `active` | `2026-04-22` | `slice-001 completed; slice-003 first pass completed; action-grouping follow-on completed; slice-004 completed; slice-005 completed; legacy cleanup and higher-level panel/shell language remain open` |
| `discussion_batch_012` | `follow-up` | `Frontend display-list baseline and list grammar` | `Keep the promoted Projects task-preview validation slice as the first formal execution entry while the broader Dashboard and AI/list convergence follow-ons remain open.` | `active` | `2026-04-23` | `slice-001 promoted to sprint_candidates; later Dashboard and AI/list follow-ons remain in discussion` |
