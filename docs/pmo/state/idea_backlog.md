...
- Current status: `parked`
- Dependencies: `A bounded follow-up discussion that decides which inconsistencies matter most first, what cross-surface interaction rule should unify Notes, Projects, task lists, and local creation-entry wrappers such as `.form-section` / `.project-form`, and whether the work should be shaped as one consistency pass or multiple smaller UI slices.`
