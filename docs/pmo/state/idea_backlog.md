...
- Problem / Opportunity: `The Notes/Projects top-level creation-entry work has landed through discussion_batch_019 and its follow-up sprints, including collection-owned + New commands, shared capture surface, embedded collection layout, and lazy Notes editor loading. The remaining broader consistency question is now about adjacent list-surface behavior that was intentionally left out: project task list density/visibility, Dashboard quick add, AI reveal/list cleanup, Format note, and any future global chat/module hover rail rules. The product would still benefit from a coherent rule for how adjacent list surfaces expose capture, reveal generated/suggested content, and keep repeated high-frequency actions discoverable without adding visual noise.`
...
- Dependencies: `A bounded follow-up discussion that decides which residual inconsistency matters most first: Dashboard quick add, project task-list density/visibility, AI reveal/list cleanup, Format note, or global chat/module hover rail rules. Do not reopen the completed Notes/Projects top-level creation-entry work unless real usage exposes a regression.`
- Risks / unknowns: `If framed too broadly, this could sprawl into a full information architecture rethink instead of a practical consistency pass. The useful boundary is now residual list-surface behavior, not the already-landed Notes/Projects creation-entry pattern.`
- Suggested next action: `Park for now. Reopen only when one residual list-surface inconsistency becomes the clear highest-friction issue, then shape that one slice rather than reopening the whole Notes/Projects creation flow.`
- Reopen trigger: `A human explicitly wants to revisit Dashboard quick add, project task-list density/visibility, AI reveal/list cleanup, Format note, global hover rail behavior, or adjacent UI work exposes one of those residual inconsistencies again.`
