...
| Runtime schema / external input validation | `backend/src/routes/requestValidation.js`, route-level body/query checks in `backend/src/routes/*.js`, `backend/src/middleware/errorBoundary.ts` | `replace` | Current ad hoc validators are doing real runtime protection; future schema guardrails may replace implementation, not responsibility. | Runtime validation must remain even with TS. Schema-library choice is a future PMO slice. |
...
| Backend current-user ownership boundary | `backend/src/middleware/currentUser.ts`, `backend/src/services/ownership.ts` | `keep` | Owns user id resolution and owner-scoped query filters used by product services. | Durable account isolation boundary; never classify as scaffolding. |
