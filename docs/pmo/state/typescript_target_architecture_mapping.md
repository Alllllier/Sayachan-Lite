...
| --- | --- | --- | --- | --- |
| Domain contracts and DTOs | Currently implicit across `frontend/src/features/**/*.api.js`, `frontend/src/services/tasks/task.rules.js`, `backend/src/models/*.ts`, `backend/src/routes/schemas/mutations.ts`, PMO baselines | `replace` | Target needs explicit types for persisted models, API responses, editable inputs, and AI DTOs instead of shape assumptions spread through API/rules/validation docs. | Do not treat Mongoose model shape as identical to public DTO shape. Needs architecture decision on local feature types vs eventual shared package. |
| Runtime schema / external input validation | `backend/src/middleware/requestBodyValidation.ts`, route-level body/query checks in `backend/src/routes/*.ts`, `backend/src/routes/schemas/mutations.ts`, `backend/src/middleware/errorBoundary.ts` | `replace` | Current Zod-backed validators and route checks are doing real runtime protection; future schema guardrails may replace implementation, not responsibility. | Runtime validation must remain even with TS. Broader schema coverage is a future PMO slice. |
...
| Backend auth/session/owner service | `backend/src/services/authService.ts`, `backend/src/routes/authRoutes.ts`, `backend/src/middleware/auth.ts`, `backend/src/models/User.ts`, `Invite.ts`, `Session.ts` | `keep` | Owns phase-one auth, invite flow, owner bootstrap, legacy data assignment, sessions, owner operations, role/status model shape. | Security/account boundary; future DTO/schema types should be conservative. |
...
| Backend route validation helper implementation | `backend/src/middleware/requestBodyValidation.ts`, `backend/src/routes/schemas/mutations.ts` | `replace` | Responsibility is durable, but implementation may evolve with runtime schema declarations aligned with DTOs. | Do not remove current validation without equivalent runtime checks. |
