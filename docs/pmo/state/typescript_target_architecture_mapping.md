...
| Backend server composition | `backend/src/server.js`, `backend/src/database.js`, `backend/src/routes/index.ts` | `keep` | Owns Koa app composition, CORS/body/auth/error middleware order, database startup, route aggregation. | Middleware order is behavior; type changes must not reorder it. |
