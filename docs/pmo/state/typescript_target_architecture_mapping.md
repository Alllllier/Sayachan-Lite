...
| --- | --- | --- | --- | --- |
| Domain contracts and DTOs | Currently implicit across `frontend/src/features/**/*.api.js`, `frontend/src/services/tasks/task.rules.js`, `backend/src/models/*.js`, `backend/src/routes/requestValidation.js`, PMO baselines | `replace` | Target needs explicit types for persisted models, API responses, editable inputs, and AI DTOs instead of shape assumptions spread through API/rules/validation docs. | Do not treat Mongoose model shape as identical to public DTO shape. Needs architecture decision on local feature types vs eventual shared package. |
| Runtime schema / external input validation | `backend/src/routes/requestValidation.js`, route-level body/query checks in `backend/src/routes/*.js`, `backend/src/middleware/errorBoundary.js` | `replace` | Current ad hoc validators are doing real runtime protection; future schema guardrails may replace implementation, not responsibility. | Runtime validation must remain even with TS. Schema-library choice is a future PMO slice. |
...
| Backend server composition | `backend/src/server.js`, `backend/src/database.js`, `backend/src/routes/index.js` | `keep` | Owns Koa app composition, CORS/body/auth/error middleware order, database startup, route aggregation. | Middleware order is behavior; type changes must not reorder it. |
| Backend auth/session/owner service | `backend/src/services/authService.ts`, `backend/src/routes/authRoutes.js`, `backend/src/middleware/auth.js`, `backend/src/models/User.js`, `Invite.js`, `Session.js` | `keep` | Owns phase-one auth, invite flow, owner bootstrap, legacy data assignment, sessions, owner operations, role/status model shape. | Security/account boundary; future DTO/schema types should be conservative. |
| Backend current-user ownership boundary | `backend/src/middleware/currentUser.js`, `backend/src/services/ownership.ts` | `keep` | Owns user id resolution and owner-scoped query filters used by product services. | Durable account isolation boundary; never classify as scaffolding. |
| Backend Notes route/service/model | `backend/src/routes/notesRoutes.ts`, `backend/src/services/notesService.ts`, `backend/src/models/Note.js` | `keep` | Durable route/service/model separation for scoped note CRUD, pinning, archive/restore, and note-origin task cascades. | Service behavior owns archive cascade semantics; route validation may be replaced later. |
| Backend Projects route/service/model | `backend/src/routes/projectsRoutes.ts`, `backend/src/services/projectsService.ts`, `backend/src/models/Project.js` | `keep` | Durable separation for scoped project CRUD, status, focus task id, pinning, archive/restore, project-task cascades. | Focus/archive behavior is architecture-sensitive. |
| Backend Tasks route/service/model | `backend/src/routes/tasksRoutes.ts`, `backend/src/services/tasksService.ts`, `backend/src/models/Task.js` | `keep` | Durable separation for scoped task reads/writes, provenance, lifecycle status, archive visibility, focus clearing. | Model/API response separation matters; task status/completed/archived typing should be explicit. |
...
| Backend route validation helper implementation | `backend/src/routes/requestValidation.js` | `replace` | Responsibility is durable, but implementation may be replaced by runtime schema declarations aligned with DTOs. | Dependency/schema choice deferred; do not remove current validation without equivalent runtime checks. |
...
| Backend `normalizeNote`, `normalizeProject`, `normalizeTask` helper pieces | `backend/src/services/taskRuntimeHelpers.ts` | `merge` | Some normalization could move into model-to-DTO adapters in a typed backend boundary. | Keep lifecycle/focus/archive helpers separate from pure DTO adapters. |
...
- Account/session/ownership boundaries: `frontend/src/stores/auth.js`, `frontend/src/services/resourceCache.js`, `backend/src/middleware/auth.js`, `backend/src/middleware/currentUser.js`, `backend/src/services/ownership.ts`, `backend/src/services/authService.ts`, auth models/routes.
