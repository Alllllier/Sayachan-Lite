...
| Backend current-user ownership boundary | `backend/src/middleware/currentUser.js`, `backend/src/services/ownership.js` | `keep` | Owns user id resolution and owner-scoped query filters used by product services. | Durable account isolation boundary; never classify as scaffolding. |
| Backend Notes route/service/model | `backend/src/routes/notesRoutes.ts`, `backend/src/services/notesService.js`, `backend/src/models/Note.js` | `keep` | Durable route/service/model separation for scoped note CRUD, pinning, archive/restore, and note-origin task cascades. | Service behavior owns archive cascade semantics; route validation may be replaced later. |
| Backend Projects route/service/model | `backend/src/routes/projectsRoutes.ts`, `backend/src/services/projectsService.js`, `backend/src/models/Project.js` | `keep` | Durable separation for scoped project CRUD, status, focus task id, pinning, archive/restore, project-task cascades. | Focus/archive behavior is architecture-sensitive. |
| Backend Tasks route/service/model | `backend/src/routes/tasksRoutes.ts`, `backend/src/services/tasksService.js`, `backend/src/models/Task.js` | `keep` | Durable separation for scoped task reads/writes, provenance, lifecycle status, archive visibility, focus clearing. | Model/API response separation matters; task status/completed/archived typing should be explicit. |
| Backend task runtime helpers | `backend/src/services/taskRuntimeHelpers.js` | `split` | Owns reusable archive filters, project-task relation filters, normalization, lifecycle derivation, focus clearing, archive/restore task cascades. | Some normalizers may merge into DTO adapters later, but focus/cascade helpers are behavior-bearing and durable. |
...
| Backend `normalizeNote`, `normalizeProject`, `normalizeTask` helper pieces | `backend/src/services/taskRuntimeHelpers.js` | `merge` | Some normalization could move into model-to-DTO adapters in a typed backend boundary. | Keep lifecycle/focus/archive helpers separate from pure DTO adapters. |
