...
- The preferred near-term placement is to keep `pin` and the AI action visible, then place the overflow trigger as the final action in the row so the secondary management actions still read as a trailing "more actions" path rather than isolating AI as the last semantic action.
...
- A later refinement has now replaced the raw punctuation-trigger assumption: the overflow path should use a proper SVG kebab/overflow icon rather than a text ellipsis glyph, because the glyph-only version feels visually weak when placed at the far right and creates ordering tension around the AI action.
- Under that refinement, the stable trigger rule is now: keep `pin` and AI visible, place the overflow trigger at the far right, and render it as a consistent SVG kebab icon rather than a literal `...` or `⋮` character.
