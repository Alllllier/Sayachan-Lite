...
Runtime state/history live in the embedded independent git repository at `.pmo_runtime/`. Stable PMO workflow docs and templates remain in the product repository.

## Standard Sequence

1. Codex prepares at most 3 bounded sprint candidates.
2. Human explicitly selects one candidate to start.
3. Codex updates `.pmo_runtime/state/current_sprint.md` from stable product-repo templates as a lightweight runtime state card.
4. Codex writes the active execution contract into `.pmo_runtime/state/execution_task.md` from stable product-repo templates.
5. Codex keeps the selected candidate visible in `.pmo_runtime/state/sprint_candidates.md` only as the selected comparison source while the sprint is active.
6. The execution worker implements only the approved slice and returns a structured report in `.pmo_runtime/state/execution_report.md`.
...
9. During closeout, Codex checks whether the sprint produced a durable decision that belongs in `.pmo_runtime/state/decision_log.md`.
...
11. PMO routes any deferred or parked follow-up into `.pmo_runtime/state/idea_backlog.md` or `.pmo_runtime/state/decision_log.md` instead of leaving it only in sprint artifacts.
...
1. Codex may activate `.pmo_runtime/state/current_sprint.md` directly from `docs/pmo/state/templates/current-sprint.active.template.md` without first creating a new discussion batch or candidate entry.
2. Codex writes a bounded execution contract into `.pmo_runtime/state/execution_task.md` from `docs/pmo/state/templates/execution-task.template.md`.
3. The execution worker still returns a structured result in `.pmo_runtime/state/execution_report.md`.
...
- `.pmo_runtime/state/sprint_candidates.md` is the comparison surface and should mainly answer:
...
- `.pmo_runtime/state/current_sprint.md` should stay lightweight and act as runtime state, not a second execution brief
- `.pmo_runtime/state/current_sprint.md` should mainly answer:
...
- a selected candidate may remain visible in `.pmo_runtime/state/sprint_candidates.md` during execution only as selected-source context
- candidate entries should not be expanded into worker instructions during execution
- a micro-fix fast path handoff may come from direct PMO activation instead of `.pmo_runtime/state/sprint_candidates.md`, but it still needs an explicit bounded contract in `.pmo_runtime/state/execution_task.md`
- active `.pmo_runtime/state/current_sprint.md` should be instantiated from `docs/pmo/state/templates/current-sprint.active.template.md`
- active `.pmo_runtime/state/execution_task.md` should be instantiated from `docs/pmo/state/templates/execution-task.template.md`
- detailed touch zones, non-goals, validation expectations, and escalation points belong in `.pmo_runtime/state/execution_task.md`
- `.pmo_runtime/state/execution_task.md` should identify where the sprint came from so discussion, backlog, and handoff stay traceable
- `.pmo_runtime/state/execution_task.md` should contain only the current active execution contract, not stacked stale tasks
- any execution worker, including delegated or sub-agent workers, should treat `.pmo_runtime/state/execution_task.md` as the default execution source once the sprint is active
- if no sprint is active, keep both `.pmo_runtime/state/current_sprint.md` and `.pmo_runtime/state/execution_task.md` in explicit `idle` state rather than silently leaving stale content
...
- only rewrite `.pmo_runtime/state/execution_task.md` during active execution when the human-review correction materially changes scope, non-goals, or validation expectations
...
`.pmo_runtime/state/execution_report.md` should be treated as the final execution record up until PMO closeout, not as a frozen first-pass snapshot.
...
After closeout, the selected candidate entry in `.pmo_runtime/state/sprint_candidates.md` should be archived into `.pmo_runtime/history/candidates/` and removed from the current candidate surface. Use `.pmo_runtime/state/current_sprint.md` for the latest closeout summary and `.pmo_runtime/history/reports/` for execution history.

After PMO reads a detailed execution report, archive that report into `.pmo_runtime/history/reports/` before resetting `.pmo_runtime/state/execution_report.md` to idle.
...
1. Read the active `.pmo_runtime/state/execution_report.md` and determine the closeout status.
2. Record the closeout judgment in `.pmo_runtime/state/current_sprint.md` as a short runtime summary, not a second full report.
3. Record the documentation-sync outcome in `.pmo_runtime/state/current_sprint.md`.
4. Update the selected entry in `.pmo_runtime/state/sprint_candidates.md`:
   - archive it into `.pmo_runtime/history/candidates/`
   - remove it from `.pmo_runtime/state/sprint_candidates.md`
5. Route any durable decisions or deferred follow-up into `.pmo_runtime/state/decision_log.md` or `.pmo_runtime/state/idea_backlog.md`.
6. If the sprint completed a work item that had been retained in `.pmo_runtime/state/idea_backlog.md`, remove that item from backlog unless it still represents unfinished future work; keep durable conclusions in `.pmo_runtime/state/decision_log.md` and keep execution history in `.pmo_runtime/history/reports/`.
7. Archive the detailed execution return into `.pmo_runtime/history/reports/` before resetting `.pmo_runtime/state/execution_report.md` to idle.
8. Reset `.pmo_runtime/state/execution_task.md` to idle when no sprint remains active, using `docs/pmo/state/templates/execution-task.idle.template.md` as the reset shape.
9. Reset `.pmo_runtime/state/execution_report.md` to idle after the detailed report has been archived, using `docs/pmo/state/templates/execution-report.idle.template.md` as the reset shape.
10. Set `.pmo_runtime/state/current_sprint.md` back to `idle` when no sprint remains active, using `docs/pmo/state/templates/current-sprint.idle.template.md` as the reset shape.
