...
- `backend/src/privateCore/bridge.ts` 仍是进入 private core 的预期公开桥接点，并承载 private-core chat execution 之上的窄公开契约。
...
| --- | --- | --- | --- | --- |
| 领域契约与 DTO | 当前隐含在 `frontend/src/features/**/*.api.js`、`frontend/src/services/tasks/task.rules.js`、`backend/src/models/*.ts`、`packages/contracts/src/product.ts`、PMO baselines 中 | `replace` | 目标需要显式类型来表达 persisted models、API responses、editable inputs、AI DTOs，而不是让 shape assumptions 分散在 API/rules/validation docs 中。 | 不要把 Mongoose model shape 当作 public DTO shape。Product request DTOs 已进入 shared contracts；response contract review 仍是单独决策。 |
| 运行时 schema / 外部输入验证 | `backend/src/middleware/route/requestBodyValidation.ts`、`backend/src/routes/*.ts` 中的 route-level body/query checks、`packages/contracts/src/product.ts`、`backend/src/middleware/app/errorBoundary.ts` | `replace` | 当前 Zod-backed validators 与 route checks 在承担真实运行时保护；未来 schema guardrails 可能替换实现，而不是替换职责。 | 即使有 TS，runtime validation 也必须保留。更广的 schema 覆盖是未来 PMO slice。 |
...
| Backend server composition | `backend/src/server.ts`、`backend/src/database.ts`、`backend/src/routes/index.ts` | `keep` | 负责 Koa app composition、CORS/body/auth/error middleware order、database startup、route aggregation。 | middleware order 是行为；类型变更不得重排。 |
| Backend auth/session/owner service | `backend/src/services/authService.ts`、`backend/src/routes/authRoutes.ts`、`backend/src/middleware/app/auth.ts`、`backend/src/http/sessionCookies.ts`、`backend/src/models/User.ts`、`Invite.ts`、`Session.ts` | `keep` | 负责 phase-one auth、invite flow、owner bootstrap、sessions、owner operations、role/status model shape。Owner bootstrap 中的 legacy product-data assignment 已退休。 | security/account boundary；未来 DTO/schema types 应保守。 |
| Backend current-user ownership boundary | `backend/src/middleware/route/currentUser.ts`、product service `{ userId }` / `{ _id, userId }` filters | `keep` | 负责 user id resolution 和 product services 显式 owner-scoped query filters。 | 持久 account isolation boundary；永远不要当作 scaffolding。 |
| Backend Notes route/service/model | `backend/src/routes/notesRoutes.ts`、`backend/src/services/notesService.ts`、`backend/src/models/Note.ts` | `keep` | scoped note CRUD、pinning、archive/restore、note-origin task cascades 的持久 route/service/model 分离。 | service behavior 拥有 archive cascade semantics；route validation 未来可被替换。 |
...
| Backend cascade/service query/domain lifecycle helpers | `backend/src/services/cascadeService.ts`、`backend/src/services/queryFilters.ts`、`backend/src/domain/lifecycle.ts` | `split` | service-owned cascade orchestration 负责 focus clearing 与 archive/restore task cascades；service query helpers 保留 services 复用的 Mongo query composition；domain lifecycle 保留可复用 rule derivation。 | cascade/query behavior 会触碰 model，属于 service orchestration；lifecycle derivation 仍是可复用 rule helper。 |
| Backend AI routes | `backend/src/routes/aiRoutes.ts` | `split` | 当前负责 note/project AI routes、provider/fallback prompt construction、persisted ownership reloads、project focus context、chat route bridge invocation 和 route-local test exports。 | 只有在未来 AI/API boundary slice 中才拆分 route-local DTO/runtime validation 与 provider/fallback helpers；必须保留 fallbacks 和 ownership checks。 |
| Public/private AI bridge | `backend/src/privateCore/bridge.ts`、`backend/private_core/sayachan-ai-core/**` boundary docs | `keep` | public bridge 是进入 private-core chat execution 的预期窄契约。 | architecture-sensitive；public mapping 可定义 contract shape，但不能定义 private-core implementation。 |
...
| Backend route validation helper implementation | `backend/src/middleware/route/requestBodyValidation.ts`、`packages/contracts/src/product.ts` | `replace` | 职责持久，但实现未来可能继续随 DTO 对齐的 runtime schema declarations 演进。 | 没有等价 runtime checks 前不能删除 current validation。 |
...
| Backend `normalizeNote`、`normalizeProject`、`normalizeTask` helper pieces | `backend/src/services/responses/productResponses.ts` | `merge` | 部分 normalization 可以继续放在 typed backend boundary 的 model-to-DTO adapters 里。 | lifecycle/focus/archive orchestration 应与 pure DTO adapters 分开保留。 |
...
- `backend/src/privateCore/bridge.ts` 与 public/private AI responsibility split。
