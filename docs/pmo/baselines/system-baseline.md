...
- Note, Project, and Task routes attach `requireCurrentUser` from `backend/src/routes/currentUser.js` before product handlers, so product handlers consume `ctx.state.userId` rather than resolving ownership themselves
