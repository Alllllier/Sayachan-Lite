...
flowchart TB
    ...
    OG --> S[".pmo_runtime/state/<br/>记录 PMO 当前认为真实的状态"]
    OG --> PO["policies/<br/>规定 PMO 如何处理判断题"]
    OG --> B["baselines/<br/>记录系统 truth 和边界"]
    OG --> H[".pmo_runtime/history/<br/>解释 PMO 运行历史"]
...
| 如果你想知道…… | 先看这里 | 再看 |
|---|---|---|
| 当前系统正在做什么 | `.pmo_runtime/state/` | `protocols/` |
| 一个话题应该怎么流转 | `protocols/` | `.pmo_runtime/state/` |
| 某个判断为什么这样做 | `policies/` | 相关 `protocols/` |
| 系统 truth / boundary 是什么 | `baselines/` | 相关 `policies/` |
| PMO 执行和候选历史在哪里 | `.pmo_runtime/history/` | `operator-guides/` |
