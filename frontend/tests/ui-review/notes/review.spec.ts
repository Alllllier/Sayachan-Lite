...
test.describe('Notes UI review', () => {
  test('captures required desktop Notes states with mocked APIs', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'New note' }).click()
    await page.getByPlaceholder('Title').fill('Canceled capture')
    await fillEditor(page, 0, 'This canceled note should not be created.')
    await page.getByRole('button', { name: 'Cancel' }).click()
    await expect(page.getByRole('dialog', { name: 'New Note' })).toHaveCount(0)
    await expect(page.getByRole('heading', { name: 'Canceled capture' })).toHaveCount(0)
    ...
    await fillEditor(page, 0, 'Updated markdown review content.')
    await editingCard.getByRole('button', { name: 'Save' }).click()
  ...
  test('captures mobile active Notes state with mocked APIs', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'New note' }).click()
    await expect(page.getByRole('dialog', { name: 'New Note' })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Close new note' })).toBeVisible()
